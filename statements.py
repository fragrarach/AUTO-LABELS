from quatro import sql_query, scalar_data, tabular_data, configuration as c
...
def plq_id_plq_qty_per(plq_id):
    sql_exp = f'SELECT (plq_qty_per)::INT FROM planning_lot_quantity WHERE plq_id = {plq_id}'
    result_set = sql_query(sql_exp, c.config.sigm_db_cursor)
...
def orl_id_orl_qty_ready(orl_id):
